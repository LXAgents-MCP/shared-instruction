...
**The forge appended a session link after posting.** `no-session-links.md` has a *Check
again after you post* section for exactly this, and it earned its place: the three bodies
left here clean, and reading them back from GitHub showed
`_Generated by [Claude Code](https://claude.ai/code/session_<id>)_` appended to #44 and
#45. Not #43 — its body had been edited afterwards to carry the chain, and the edit
replaced the appended footer rather than preserving it. So the footer arrives on create
and survives until something overwrites the body.

Both were edited in place and re-read to confirm the line is gone; no history rewrite was
needed, because a posted body is the easy case. **The practical lesson for the next
session: creating a pull request here is not finished until you have read the stored body
back.** A create call that returns success has told you nothing about what was stored, and
the local message you sent is not evidence.
