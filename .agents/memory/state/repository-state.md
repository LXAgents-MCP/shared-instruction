...
**Structure.** `content/` holds the 24 published instruction files. `.agents/` holds
...
**Surface.** 2 prompts (`agents-setup`, `check-duplicate-agents-instruction`), 25
resources (24 instruction files plus `agents://manifest.json`), and 4 read-only tools
...
**Version.** `0.1.0`. Releases so far: `0.0.0` (initial set), `0.1.0` (tool surface).

**Tests.** 45, all passing, across registry, server, http, manifest, and tools.
